...
class AbstractCompositionBayesianOptimization(ABC):
    def __init__(self,
                 elem_num: int = 4,
                 init_rand_num: int = 15,
                 total_exp_num: int = 65,
                 init_seed: int  = 0,
                 elems: List = None
                 ):
        self._internal_func = get_mo_ground_truth_func()
        self._gpr = None
        self._gpr_noise_level = 1e-6

        self._elem_num = elem_num

        original_elems = ['C', 'Al', 'V', 'Cr', 'Mn', 'Fe', 'Co', 'Ni', 'Cu', 'Mo']
        self._all_elem_names = ['V', 'Cr', 'Mn', 'Fe', 'Ni', 'Cu', 'Mo'][:elem_num]        # 5
        if elems: self._all_elem_names = elems

        elem_idx_dict = dict(zip(original_elems, range(len(original_elems))))
        idx_list = np.array([elem_idx_dict[_elem] for _elem in self._all_elem_names])

        self._bounds = np.vstack(
            [list(COMP_LIMITS[_i]) for _i in idx_list]
        )
        ...
        def _f(comp):
            _tmp_comp = np.zeros(len(original_elems))
            for _i, _c in zip(idx_list, comp): 
                _tmp_comp[_i] = _c 
            return self._internal_func(_tmp_comp)
        self._ground_truth_func = _f
    ...
    def rejection_sample_compositions(self, 
        ...
        for i in range(self._elem_num):
            _min, _max = self._bounds[i][0], self._bounds[i][1]
        ...
        while len(_comp_buff) < num:
            ...
            if _last_c < self._bounds[-1][0] or _last_c > self._bounds[-1][1]:
                _flag = False       # early stop
            else:
                _comp[-1] = _last_c
            if _flag:
    ...
    def save(self):
        joblib.dump(
            ...
            f'{str(uuid.uuid4())[:8]}-enum-{self._init_seed}-{self._init_rand_num}-{self._total_exp_num}.pkl'
        )
