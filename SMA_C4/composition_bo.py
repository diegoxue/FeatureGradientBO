...
class AbstractCompositionBayesianOptimization(ABC):
    def __init__(self,
        ...
        self._bounds = np.array(COMP_LIMITS[:elem_num])     # TODO: round, 添加注释
    ...
    def init_orthogonal_compositions(self):
        ...
        combos = itertools.product(
            np.linspace(0.45, 0.55, 3),
            np.linspace(CU_MIN, CU_MAX, 3),
            np.linspace(HF_MIN, HF_MAX, 3),
        )
    ...
    def feature_selection(self):
        ...
        _feature_funcs = [weight_avg_func, delta_func, max_func, min_func,
                          range_func, maxc_func, minc_func, rangec_func]
        _n_comps, _n_elem_feat, _n_feat_func = len(_comps), len(self._elem_features), len(_feature_funcs)
            ...
            for j in range(_material_feature.shape[-1]):
                ''' 
                     _material_feature:
                        ...
                        same column : same (elemental feature, feature transformation function) combo.

                        First iterate different <elemental feature> for the same <feature transformation function>.
                        Then iterate different <feature transformation function>.
                '''
                _elem_feat_idx, _feat_func_idx = j % _n_elem_feat, j // _n_elem_feat
