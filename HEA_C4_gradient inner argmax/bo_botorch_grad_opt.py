...
class GradOptBayesianOptimization(AbstractCompositionBayesianOptimization):
    ...
    def save(self):
        joblib.dump(
            ...
            f'{str(uuid.uuid4())[:8]}-hea_grad_c{self._elem_num}-{self._init_seed}-exp-{self._init_rand_num}-{self._total_exp_num}-inner-{self._default_inner_opt_num}-q-{self._batch_q}.pkl'
        )
